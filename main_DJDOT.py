    ...
    for e in range(1, NUM_EPOCHS + 1):
        ...
        if the_current_loss >= the_last_loss:
            ...
            if test_f1 <= val_acc_mat[0]:
                ...
                torch.save(net.state_dict(), config["model_path"] + "f1_djdot_40epoch.pt")
            print("trigger times:", trigger_times)
            if trigger_times == patience:
                print("Early stopping!\nStart to test process.")
                torch.save(net.state_dict(), config["model_path"] + "es_djdot_40epoch.pt")
        else:
...
#         # lrs.append(optimizer.param_groups[0]["lr"])
    ...
    print("finished")
    torch.save(net.state_dict(), config["model_path"] + "DA_djdot_40epoch.pt")
    gc.collect()
